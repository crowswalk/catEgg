...
public class camFollow : MonoBehaviour { //camera following behavior, doesn't cross world bounds
    ...
    public Vector3 cutSceneTransform;
    private Transform followTransform; //transform of object to follow
    ...
    void FixedUpdate() {
        if (endScene.sceneActive) {
            gameObject.transform.position = cutSceneTransform;
        } else {
